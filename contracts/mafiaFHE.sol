...
contract Mafia is EIP712WithModifier {
    ...
    constructor(address _creator) EIP712WithModifier("Authorization token", "1") {
        owner = _creator;
        joinGame(_creator);
    }
    ...
    function initializeGame() public {
        ...
        for (uint8 i = 0; i < playersList.length; i++) {
            players[playersList[i]] = Player(i, playersList[i], TFHE.add(roles[i], TFHE.asEuint8(1)), true);
            idToPlayer[i] = Player(i, playersList[i], TFHE.add(roles[i], TFHE.asEuint8(1)), true);
        }
        roundCount++;
        gameState = 1;
    ...
    function checkIfMafiaKilled() internal {
        ...
        if (tieExists) {
            ...
            if (role == 1 && !tieExists) {
                ...
                emit Killed(idToPlayer[playerIdWithLargestVoteCount].playerAddress);
                if (playerCount > 2) {
                    emit CheckMafia(false);
                    _resetDay();
                } else {
